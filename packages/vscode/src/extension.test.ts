...
describe("VSCode Extension", () => {
  ...
  it("should handle promptshield.fixWithAI command", async () => {
    ...
    vi.mocked(vscode.Uri.parse).mockReturnValue({} as any);
    ...
    if (handler) {
      ...
      await handler("file:///test.ts", threat);

      expect(vscode.lm.selectChatModels).toHaveBeenCalled();
      expect(mockModel.sendRequest).toHaveBeenCalled();
      // Check if edit applied - we need to spy on WorkspaceEdit if possible or applyEdit
      expect(vscode.workspace.applyEdit).toHaveBeenCalled();
    }
  });

  it("should handle promptshield.fixWithAI command with no models", async () => {
    const { activate } = await import("./extension");
    activate(context);
    ...
    if (handler) {
      const threat = { loc: { index: 0 }, offendingText: "bad" };
      await handler("file:///test.ts", threat);

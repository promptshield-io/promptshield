...
export const showHelp = () => {
  console.log(`
Usage: promptshield-cli [options]
...
export const main = async (args: string[] = process.argv.slice(2)) => {
  ...
  if (init) {
    await writeFile(
      resolve(findProjectRoot(), configFile || "promptshield-cli.config.json"),
      JSON.stringify(DEFAULT_CONFIG, null, 2),
    );
    console.log(
      `Created default config file at ${configFile || "promptshield-cli.config.json"}`,
    );
    return;
  }
